...
class ProximityManager: NSObject {
    ...
    fileprivate func updateBeaconsInRange() {
        let monitoredIdentifiers = Array<String>(self.monitoringManager.monitoredIdentifiers)
        let beaconIdentifiersInRange = monitoredIdentifiers.filter { [weak self] (identifier) -> Bool in
